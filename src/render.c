...
void render_frame(const GameContext *ctx) {
	...
	for (uint8_t i = 0; i < ctx->wireframe.instance_count; ++i) {
		const Instance3D *inst = &ctx->wireframe.instances[i];

		// if(i > 17) continue; //  Used to limit instances drawn per frame for testing --- IGNORE ---

		// for this code all objects are the same model
		transform_instance_to_screen(inst, &ctx->wireframe.camera, i==0); 
		out_edge_count = get_screen_edges(inst->model, 3);  // set to fixed color 'color' could be used here
