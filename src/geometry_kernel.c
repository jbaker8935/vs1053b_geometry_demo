...
void setup_object_params(uint8_t pitch, uint8_t yaw, uint8_t roll, uint8_t scale, int16_t pos_x, int16_t pos_y,
                         int16_t pos_z) {
...
void setup_object_pos(int16_t pos_x, int16_t pos_y, int16_t pos_z) {
    vs1053_write_mem(VS_GEOM_OBJ_POS_X, pos_x);
    vs1053_write_sci(SCI_WRAM, pos_y);
    vs1053_write_sci(SCI_WRAM, pos_z);
}

// Setup camera transformation parameters
void setup_camera_params(uint8_t pitch, uint8_t yaw, uint8_t roll, int16_t pos_x, int16_t pos_y, int16_t pos_z) {
    vs1053_write_mem(VS_GEOM_CAM_PITCH_YAW, (uint16_t)pitch << 8 | yaw);
    vs1053_write_sci(SCI_WRAM, ((uint16_t)roll << 8) |
...
// Setup camera transformation parameters
void setup_projection_params(int16_t focal, int16_t half_w, int16_t half_h, int16_t near_z) {
    vs1053_write_mem(VS_GEOM_ENABLE_PROJECT, 0x0001);  // Enable projection
    vs1053_write_sci(SCI_WRAM, 0x0001);                // Enable clipping
...
void setup_projection_disable(void) {
    vs1053_write_mem(VS_GEOM_ENABLE_PROJECT, 0x0000);  // Disable projection (passthrough)
    vs1053_write_sci(SCI_WRAM, 0x0000);                // Disable clipping
}
...
void get_view_z(int16_t *out_zbuffer, uint8_t n_verts) {
    uint16_t addr = VS_GEOM_OUTPUT_VERT + 2;
    while (n_verts--) {
        /* output vz is a signed Q0 view-space Z (camera looks toward -Z) */
        *out_zbuffer++ = (int16_t)vs1053_read_mem(addr);  // vz (signed)
        addr += 3;
    }
}
...
uint8_t screen_y8[VS_GEOM_MAX_VERTICES];
uint16_t clip_verts_y[VS_GEOM_MAX_VERTICES];
int16_t zbuffer[VS_GEOM_MAX_VERTICES];
int16_t depth_metric[VS_GEOM_MAX_VERTICES];
...
uint8_t get_screen_edges_with_depth(Model3D *model, uint8_t near_color, uint8_t far_color) {
    ...
    uint8_t n_input = model->vertex_count;  // vs1053_read_mem(VS_GEOM_N_VERTICES);

    // Read number of output edges
    uint8_t edge_count = vs1053_read_mem(VS_GEOM_N_OUTPUT_EDGES);
    ...
    int16_t max_metric = INT16_MIN;
    int16_t min_metric = INT16_MAX;

    vs1053_write_sci(SCI_WRAMADDR, VS_GEOM_SCREEN_COORDS);

    for (uint8_t i = 0; i < n_input; ++i) {
        screen_x[i] = vs1053_read_sci(SCI_WRAM);
        uint16_t sy = vs1053_read_sci(SCI_WRAM);
        screen_y[i] = sy;
        screen_y8[i] = (uint8_t)sy;
        int16_t metric = (int16_t)(sy >> 8) - 128;
        depth_metric[i] = metric;
    ...
    if (n_clip == 0 && model->edge_count == edge_count) {
        // Fast no-clip/cull path: depth metric is packed in high byte of Y.
        ...
        while (e--) {
            ...
            uint8_t sy0 = screen_y8[v0];
            uint16_t sx1 = screen_x[v1];
            uint8_t sy1 = screen_y8[v1];

            int16_t edge_mid_delta = depth_metric[v0] + depth_metric[v1];
            uint8_t edge_color = (edge_mid_delta >= 0) ? near_color : far_color;

            add_line_to_list(sx0, sy0, sx1, sy1, edge_color);
        }
        ...
        for (uint8_t e = 0; e < edge_count; ++e) {
            ...
            uint8_t edge_color = (flags & VS_GEOM_EDGE_NEAR) ? near_color : far_color;
            add_line_to_list((uint16_t)sx0, (uint8_t)sy0, (uint16_t)sx1, (uint8_t)sy1, edge_color);
        }
    }
